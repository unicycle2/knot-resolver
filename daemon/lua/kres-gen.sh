...
exec > $(dirname $0)/kres-gen.lua

GEN_CDEFS=${MESON_SOURCE_ROOT:-.}/scripts/gen-cdefs.sh
...
genResType() {
	echo "$1" | ${GEN_CDEFS} libkres types
}
...
## libknot API
${GEN_CDEFS} libknot functions <<-EOF
# Domain names
	knot_dname_size
	knot_dname_from_str
	knot_dname_to_str
...
## libkres API
${GEN_CDEFS} libkres functions <<-EOF
# Resolution request
	kr_resolve_plan
	kr_resolve_pool
# Resolution plan
